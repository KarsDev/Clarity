...
public enum TokenType {
    ...
    KEYWORD("\\b(class|var|constructor|local|fn|native|if|else|return|new|void|include|static|const|compiled|null|for|while|select|when|default|break|continue|float|int|inherits|assert|is|arr|str|enum)\\b"), // All keywords
    BOOLEAN("\\b(true|false)\\b"), // Matches booleans
    STRING("\"(\\\\.|[^\"\\\\])*\"|'(\\\\.|[^'\\\\])*'"), // Matches anything between " and " or ' and ', including escaped characters
    OPERATOR("\\^\\^|\\|\\||<<|>>|\\+\\+|--|[+\\-*/%=<>!]=?|==|\\.\\.\\.|\\.|&&|\\?|:|\\^|&|\\|"), // Adjusted order to handle || before |
